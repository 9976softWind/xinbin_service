...
import com.wims.iot.converter.RoleConverter;
import com.wims.iot.mapper.SysRoleMapper;
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
    ...
    @Override
    public List<AuthorityDto> mapToGrantedAuthorities(Integer userId) {
        Set<String> permissions = new HashSet<>();
        List<SysRole> list = this.baseMapper.findByUserId(userId);
        for (SysRole s : list) {
            if ("ROOT".equals(s.getCode())) {
                permissions.add("ROOT");
                return permissions.stream().map(AuthorityDto::new)
                        .collect(Collectors.toList());
            }
            Set<String> collect = s.getMenus().stream().map(SysMenu::getPerm)
